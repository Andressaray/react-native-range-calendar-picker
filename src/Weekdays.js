...
export const StaticWeekdays = (props) => {
    const {weekDaysLocale, width, type, staticWeekdaysBackColor, staticWeekdaysTextColor, weak} = props;
            ...
            {weekDays.map((dayName, i) => {
                return (
                    <View key={i} style={[styles.weekDayNamesItem, {width: (width) / 7, height: (width) / 7}]}>
                        <Text
                            style={[{color: '#444'}, staticWeekdaysTextColor && {color: staticWeekdaysTextColor}, {...weak.textStyle}]}>{dayName}</Text>
                    </View>
                );
            })}
        </View>
